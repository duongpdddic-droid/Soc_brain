...
export function handlePassBranch({ result, sessionPath, stateDir, identityHash: id }) {
  ...
  // The decision carries the review verdict, NOT the delivery metadata.
  // result.delivery is delivery status (shipped, cleanup) — not review evidence.
...
// Contract: result = { state: 'BLOCKED', terminalize, decision, loopToken }
...
export function handleBlockedBranch({ result, sessionPath, stateDir, identityHash: id }) {
  ...
  // The BLOCKED result now carries `decision` (the review verdict with
  // findings/evidenceRequests) — added by decide() to the BLOCKED return.
  // Fallback to terminalize.evidence for legacy compatibility.
  const reason = result.reason || null;
  const decision = result.decision
    || (result.terminalize && result.terminalize.evidence)
    || null;
  const findings = (decision && decision.findings) || [];
  const evidenceRequests = (decision && decision.evidenceRequests) || [];
