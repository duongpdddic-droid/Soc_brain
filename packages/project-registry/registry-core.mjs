// registry-core.mjs - Shared canon/legacy primitives for the Canonical Project Registry
// (Soc_brain Issue #17). Bottom of the module graph: imported by registry-storage.mjs,
// reconcile-engine.mjs and the test injector. Contains NO reconciliation orchestration
// and NO dependency-injection entry, so shipping it exposes no injectable authority seam.
// RFC 8785 JCS via canonical-jcs.mjs.
import {
  readFileSync, writeFileSync, existsSync, mkdirSync, renameSync, lstatSync,
  openSync, closeSync, fsyncSync, unlinkSync, rmSync, rmdirSync, realpathSync, linkSync,
} from 'node:fs';
...
// ---- Legacy reconciliation pure helpers --------------------------------------
// Dependency-free decision helpers used by BOTH the shipped production
// reconcile-engine.mjs and the test-only injector. No dep-taking function, so
// this module exposes no injectable authority seam.
...
function legacyNormalizedProjects(legacy) {
  ...
  if (!list) return { ok: false, errors: ['legacy.projects khong phai array'], norms: out };
  for (let i = 0; i < list.length; i++) {
    const m = list[i];
    if (!m || typeof m !== 'object' || Array.isArray(m)) { errors.push(`legacy.projects[${i}] malformed (not object)`); continue; }
    const pid = typeof m.projectId === 'string' ? m.projectId : null;
    if (!pid || !PROJECT_ID_RE.test(pid)) { errors.push(`legacy.projects[${i}] projectId khong hop le`); continue; }
    const repo = typeof m.repository === 'string' ? m.repository : '';
    if (!repo || !REPO_RE.test(repo)) { errors.push(`legacy.projects[${i}] repository khong hop le`); continue; }
    if (seenPid.has(pid)) { errors.push(`REGISTRY_DUPLICATE_IDENTITY: legacy duplicate projectId '${pid}'`); continue; }
    seenPid.add(pid);
    const repoKey = repo.toLowerCase();
    if (seenRepo.has(repoKey)) { errors.push(`REGISTRY_DUPLICATE_IDENTITY: legacy duplicate repository '${repo}'`); continue; }
    seenRepo.add(repoKey);
    out.set(pid, { projectId: pid, canonicalRepository: repo,
...
function verifyReconciliation(legacyNorms, canonProjects) {
  ...
  for (const [pid, p] of Object.entries(canonProjects)) {
    ...
    if (typeof repo !== 'string' || !REPO_RE.test(repo)) { errors.push(`project '${pid}' canonicalRepository khong hop le`); continue; }
    if (repoOwner.has(repo)) errors.push(`REGISTRY_DUPLICATE_IDENTITY: repository '${repo}' trung giua '${repoOwner.get(repo)}' va '${pid}'`);
    else repoOwner.set(repo, pid);
  }
  for (const [pid, p] of Object.entries(canonProjects)) {
    const l = legacyNorms.get(pid);
    if (!l) { errors.push(`project '${pid}' khong co trong legacy; khong phai verified superset`); continue; }
    if (l.canonicalRepository !== p.canonicalRepository) errors.push(`project '${pid}' legacy repo '${l.canonicalRepository}' != canonical repo '${p.canonicalRepository}'`);
  }
  for (const [pid, l] of legacyNorms) {
    if (canonProjects[pid]) continue;
    if (repoOwner.has(l.canonicalRepository)) errors.push(`REGISTRY_DUPLICATE_IDENTITY: repository '${l.canonicalRepository}' trung giua '${repoOwner.get(l.canonicalRepository)}' va '${pid}'`);
    else repoOwner.set(l.canonicalRepository, pid);
  }
  return errors.length ? { ok: false, errors } : { ok: true };
}

function buildReconciledEntry(l, canonicalRoot, now) {
  const entry = { projectId: l.projectId, canonicalRepository: l.canonicalRepository, canonicalRoot, status: 'active', registeredAt: now.toISOString(), capabilities: [] };
...
function buildReconciledNext(canonData, legacyNorms, roots, now) {
  ...
  for (const [pid, l] of legacyNorms) {
    ...
    if (typeof rootRaw !== 'string' || !rootRaw) { errors.push(`project '${pid}' thieu canonicalRoot de them vao canonical; cung cap qua roots`); continue; }
...
function tombstoneFor(data, registryPath, legacyPath, now, owner, legacyDigest) {
  void owner;
  return { canonicalPath: registryPath, contentDigest: data.contentDigest, legacyDigest, migratedAt: now.toISOString(), migrationVersion: MIGRATION_CONTRACT_VERSION, legacyPath, reconcile: true };
}

function tombstoneEqual(a, b) {
  return a.canonicalPath === b.canonicalPath && a.contentDigest === b.contentDigest && a.legacyDigest === b.legacyDigest && a.migrationVersion === b.migrationVersion && a.legacyPath === b.legacyPath && a.reconcile === b.reconcile;
}
...
function assertNoReparseSegments(p) {
  if (!isAbsolute(p)) return { ok: false, code: 'RECONCILIATION_TOMBSTONE_PATH_ESCAPE', errors: ['tombstone directory phai la absolute'] };
  const { root } = parse(p);
  ...
  for (const seg of segs) {
    cur = join(cur, seg);
    let st;
    try { st = lstatSync(cur); } catch { continue; }
    if (st.isSymbolicLink()) return { ok: false, code: 'RECONCILIATION_TOMBSTONE_PATH_ESCAPE', errors: [`path segment '${cur}' la symlink/junction; refuse to write tombstone`] };
  }
  return { ok: true };
}
// ---- Tombstone atomic + create-if-absent publication (REV-148) ----------------
// Write a fsynced temp in the SAME directory, then publish with create-if-absent
// semantics via a same-volume HARD LINK (linkSync). linkSync is atomic and fails
// with EEXIST if the target already exists -> the target is never overwritten. If a
// target appears between temp write and publication (race), we read+validate it:
// identical => idempotent success; different => conflict. Directory fsync + final
// read-back preserved. Falls back to an exclusive-create ('wx') adapter on platforms
// that do not support hard links.

function createTombstoneTemp(tombstone, dir) {
...
function resolveTombstoneRace(tombPath, tombstone) {
  let existing;
  try { existing = JSON.parse(readFileSync(tombPath, 'utf8')); }
  catch { return { ok: false, io: false, conflict: true, errors: ['target appeared during publication but malformed; fail-closed, no overwrite'] }; }
  if (tombstoneEqual(existing, tombstone)) return { ok: true, exists: true, data: existing };
  return { ok: false, io: false, conflict: true, errors: ['target appeared during publication with different content; conflict, no overwrite'] };
}

function publishExclusiveCreate(tempPath, tombPath, tombstone) {
  const bytes = Buffer.from(JSON.stringify(tombstone, null, 2), 'utf8');
  let fd;
  try { fd = openSync(tombPath, 'wx'); }
  catch (e) {
    if (e.code === 'EEXIST') return resolveTombstoneRace(tombPath, tombstone);
    return { ok: false, io: true, errors: ['tombstone exclusive-create failed: ' + e.message] };
  }
  let ok = true;
  try {
    writeFileSync(fd, bytes, 'utf8');
    const r = tryFsync(fd);
    if (!r.ok && r.error) ok = false;
  } catch { ok = false; }
  finally { try { closeSync(fd); } catch { /* ignore */ } }
  if (!ok) {
    try { unlinkSync(tombPath); } catch { /* ignore */ }
    return { ok: false, io: true, errors: ['tombstone exclusive-create write/fsync failed'] };
  }
  try { unlinkSync(tempPath); } catch { /* ignore */ }
  return { ok: true, published: true };
}

function publishTombstoneNoClobber(tempPath, tombPath, tombstone) {
  try {
    linkSync(tempPath, tombPath);
  } catch (e) {
    if (e.code === 'EEXIST') return resolveTombstoneRace(tombPath, tombstone);
    if (e.code === 'EPERM' || e.code === 'ENOTSUP' || e.code === 'ENOSYS' || e.code === 'EXDEV' || e.code === 'EACCES' || e.code === 'EINVAL') {
      return publishExclusiveCreate(tempPath, tombPath, tombstone);
    }
    return { ok: false, io: true, errors: ['tombstone hard-link publish failed: ' + e.message] };
  }
  try { unlinkSync(tempPath); } catch { /* ignore */ }
  return { ok: true, published: true };
}

function readTombstoneBack(tombPath, tombstone) {
  let rb;
...
function writeTombstoneAtomic(tombstone, tombPath) {
  // REV-143/144 preflight: existing non-file / malformed / conflicting -> fail-closed.
  if (existsSync(tombPath)) {
    let st;
    try { st = lstatSync(tombPath); } catch { /* fall to io */ }
    if (!st || st.isDirectory() || (!st.isFile() && !st.isSymbolicLink())) {
      return { ok: false, io: true, errors: [`tombstone path '${tombPath}' ton tai nhung khong phai file; refuse to clobber`] };
    }
    let existing;
    try { existing = JSON.parse(readFileSync(tombPath, 'utf8')); }
    catch { return { ok: false, io: false, conflict: true, errors: ['tombstone ton tai nhung malformed; fail-closed, khong overwrite'] }; }
    if (tombstoneEqual(existing, tombstone)) return { ok: true, exists: true, data: existing };
    return { ok: false, io: false, conflict: true, errors: ['tombstone ton tai voi noi dung khac; fail-closed, khong overwrite'] };
  }
  const dir = dirname(tombPath);
  const t = createTombstoneTemp(tombstone, dir);
  if (!t.ok) return t;
  const p = publishTombstoneNoClobber(t.tempPath, tombPath, tombstone);
  if (!p.ok) {
    try { unlinkSync(t.tempPath); } catch { /* ignore */ }
    return p;
  }
  if (p.exists) try { unlinkSync(t.tempPath); } catch { /* ignore */ }
  fsyncDirOrSkip(dir);
  const rb = readTombstoneBack(tombPath, tombstone);
  if (!rb.ok) return rb;
  return { ok: true, data: rb.data, exists: !!p.exists };
}
...
export {
  createTombstoneTemp, publishTombstoneNoClobber, writeTombstoneAtomic,
  readLegacyBytesDefault, sha256, legacyNormalizedProjects, verifyReconciliation,
  buildReconciledEntry, buildReconciledNext, tombstoneFor, tombstoneEqual,
  assertNoReparseSegments,
};
