...
// still boot, expose exactly 3 Broker tools and serve status/diff/run.
{
  ...
  try {
    ...
    if (result.ok) {
      ...
      eq('mcp-int tools length', byId.get(2).result.tools.length, 7);
      eq('mcp-int tool names', JSON.stringify(byId.get(2).result.tools.map((t) => t.name).sort()), JSON.stringify(['soc_broker_block_task', 'soc_broker_commit', 'soc_broker_diff', 'soc_broker_finish_task', 'soc_broker_request_human_gate', 'soc_broker_run_registered_test', 'soc_broker_status']));
