#!/usr/bin/env node
// idle-supervisor.test.mjs — Idle Sleep Supervisor regression coverage.
...
// INJECTED fake power executor — an automated test can never sleep the host.
...
import {
  ...
  readIdleSleepConfig, localPolicyMode, scanCanonicalActivity,
  createIdleSupervisor, readSleepEvidence, persistSleepEvidence,
} from '../packages/idle-supervisor/idle-supervisor.mjs';
import { createSupervisorRuntime, spawnIdleSupervisor } from '../packages/idle-supervisor/run.mjs';
...
// Fake power executor: counts requests; NEVER touches the OS.
function fakeDeps({ userIdleMs = 0, bootId = 'boot-1' } = {}) {
  const calls = [];
  return {
    calls,
    readUserIdleMs: () => userIdleMs,
    readBootId: () => bootId,
    requestSleep: () => { calls.push({ action: 'SLEEP', at: new Date().toISOString() }); return { ok: true, action: 'SLEEP' }; },
  };
}
...
isEq('supervisor states', SUPERVISOR_STATES,
  ['DISABLED', 'BUSY', 'WAIT_USER_IDLE', 'IDLE_COUNTDOWN', 'SLEEP_ELIGIBLE', 'SLEEP_REQUESTED', 'SLEEP_DENIED_UNKNOWN_ACTIVITY']);
tru('active states frozen', Object.isFrozen(ACTIVE_LOOP_STATES) && Object.isFrozen(INACTIVE_STATES));

// ---- config ------------------------------------------------------------------------
eq('explicit enable', readIdleSleepConfig({ SOC_IDLE_SLEEP: '1' }).config.enabled, true);
eq('not enabled by default', readIdleSleepConfig({}).config.enabled, false);
eq('bad night clock rejected', readIdleSleepConfig({ SOC_IDLE_SLEEP: '1', SOC_IDLE_SLEEP_NIGHT_START: '25:00' }).ok, false);
eq('bad grace rejected', readIdleSleepConfig({ SOC_IDLE_SLEEP: '1', SOC_IDLE_SLEEP_DAY_GRACE_MIN: '0' }).ok, false);
...
// ---- regression 1: EXECUTING -> no sleep ----------------------------------------------
{
  const S = mkStateDir();
...
// ---- regression 4: DELIVERING -> no sleep -----------------------------------------------
{
  const S = mkStateDir();
...
// ---- regression 5: rework + recovery states -> no sleep ----------------------------------
{
  const S = mkStateDir();
...
// ---- regression 6: only terminal/gate states -> eligible ---------------------------------
{
  ...
  eq('r6 eligible', r.state, 'SLEEP_ELIGIBLE');
  const fin = r.finalize(a);
  eq('r6 finalize ok', fin.ok, true);
  eq('r6 evidence event', fin.evidence.event, 'SLEEP_IDLE_CONFIRMED');
  ...
  eq('r6 evidence sleepRequested', fin.evidence.sleepRequested, true);
  eq('r6 evidence powerAction', fin.evidence.powerAction, 'SLEEP');
  ...
  eq('r6 state requested', fin.state, 'SLEEP_REQUESTED');
}

// ---- regression 7: DAY + userIdle < 20m -> WAIT_USER_IDLE, no sleep ----------------------
{
  const S = mkStateDir();
  const sup = createIdleSupervisor({ config: CONFIG, stateDir: S });
  const t = at(12, 0);
...
// ---- regression 8: DAY + userIdle >= 20m + no active work -> sleep once -------------------
{
  const S = mkStateDir();
  ...
  const t = rt.oneTick();
  eq('r8 state', t.state, 'SLEEP_REQUESTED');
  eq('r8 sleep exactly once', deps.calls.length, 1);
  eq('r8 fake executor only', deps.calls[0].action, 'SLEEP');
  eq('r8 evidence persisted before OS call', readSleepEvidence({ stateDir: S }).evidence.event, 'SLEEP_IDLE_CONFIRMED');
}
...
// ---- regression 9: NIGHT 00:01 clean -> countdown 10m ------------------------------------
{
  ...
  eq('r9 eligible at 10m', sup.tick({ activity: scanAt(S, t10), userIdleMs: 0, now: t10.getTime() }).state, 'SLEEP_ELIGIBLE');
}
...
// ---- regression 10: NIGHT ignores fresh user activity ------------------------------------
{
  ...
  const t2 = at(1, 10);
  eq('r10 night grace holds with user activity', sup.tick({ activity: scanAt(S, t2), userIdleMs: 5 * MIN, now: t2.getTime() }).state, 'SLEEP_ELIGIBLE');
}
...
// ---- regression 11: task appears during night countdown -> reset --------------------------
{
  ...
  eq('r11 eligible after fresh 10m', sup.tick({ activity: scanAt(S, t15), userIdleMs: 0, now: t15.getTime() }).state, 'SLEEP_ELIGIBLE');
}
...
// ---- regression 13: 06:00 flips to DAY policy ----------------------------------------------
{
  ...
  eq('r13 day flip eligible', sup.tick({ activity: scanAt(S, t1), userIdleMs: 8 * 60 * MIN, now: t1.getTime() }).state, 'SLEEP_ELIGIBLE');
...
// ---- regression 14: final read-back sees active work -> abort -------------------------------
{
  ...
  eq('r14 eligible', r.state, 'SLEEP_ELIGIBLE');
  ...
  eq('r14 abort code', fin.code, 'SLEEP_ABORTED_ACTIVE_WORK');
  eq('r14 no evidence written', readSleepEvidence({ stateDir: S }).evidence, null);
  rmSession(S, 1018);
}

// ---- regression 15: UNKNOWN canonical state -> SLEEP_DENIED_UNKNOWN_ACTIVITY ----------------
{
  const S = mkStateDir();
  ...
  const sup = createIdleSupervisor({ config: CONFIG, stateDir: S });
  const r = sup.tick({ activity: a, userIdleMs: 60 * MIN, now: t.getTime() });
  eq('r15 denied', r.state, 'SLEEP_DENIED_UNKNOWN_ACTIVITY');
...
// ---- regression 16: supervisor restart -> no duplicate Sleep -------------------------------
{
  const S = mkStateDir();
  const t0 = at(15, 0);
  ...
  eq('r16 suppressed state', t.state, 'SLEEP_REQUESTED');
...
{
  const S = mkStateDir();
  persistSleepEvidence({ stateDir: S, evidence: {
  ...
  const t1 = rt.oneTick(); // pending evidence from before "sleep" -> no new request
  eq('r17 pending held', t1.state, 'SLEEP_REQUESTED');
  now += 10 * MIN; // 10m with no tick = machine was asleep and just resumed
  const t2 = rt.oneTick(); // resume gap -> reinit: evidence result + fresh windows
  eq('r17 reinit -> fresh countdown', t2.state, 'IDLE_COUNTDOWN');
  eq('r17 resume result recorded', readSleepEvidence({ stateDir: S }).evidence.result, 'SLEEP_RESUMED');
...
// ---- regression 18: processes exist but no canonical active work -> NOT active -------------
{
  ...
  const sup = createIdleSupervisor({ config: CONFIG, stateDir: S });
  eq('r18 eligible', sup.tick({ activity: a, userIdleMs: 30 * MIN, now: t.getTime() }).state, 'SLEEP_ELIGIBLE');
}
...
// ---- regression 19: supervisor never mutates canonical/GitHub state ------------------------
{
  ...
  tru('r19 evidence confined to idle-supervisor dir',
    fs.existsSync(path.join(S, 'idle-supervisor', 'sleep-evidence.json')));
}
...
// ---- regression 20: low resource footprint -------------------------------------------------
{
  ...
  eq('r20 cooldown: one sleep decision per window, no spam', deps.calls.length, 1);
...
eq('companion spawn: enabled but no entry -> null',
  spawnIdleSupervisor({ repoRoot: path.join(TMP, 'no-such-root'), env: { SOC_IDLE_SLEEP: '1' } }), null);
