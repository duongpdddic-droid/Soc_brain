...
// ---- G. human-first formatter (rev-2 req E) --------------------------------------
{
  ...
  falsy('G TASK_STARTED no longer uses the sleep-inducing phrase', t1.includes('Bạn không cần làm gì'));
  tru('G TASK_STARTED action points to concrete follow-up reporting', t1.includes('Theo dõi các bước tiếp theo qua báo cáo'));
  ...
  // G3: TASK_BLOCKED must show the REAL objective + the concrete error code.
  const blockedRich = buildTelegramText({
    event: 'TASK_BLOCKED', session: sess,
    note: 'Review FAIL_CLOSED',
    objective: 'Wire granular FSM telemetry into Telegram dispatcher',
    pr: { present: true, number: 99, title: 'telemetry' },
    errorCode: 'VERIFY_FAILED:FULL_SUITE_1_FAIL',
  });
  tru('G3 blocked shows real objective with explicit label', blockedRich.includes('Mục tiêu thực tế: Wire granular FSM telemetry into Telegram dispatcher'));
  tru('G3 blocked shows the concrete error code', blockedRich.includes('Mã lỗi: VERIFY_FAILED:FULL_SUITE_1_FAIL'));
  tru('G3 objective appears before the what-happened line', blockedRich.indexOf('Mục tiêu thực tế:') < blockedRich.indexOf('Task bị chặn'));
  tru('G3 error code appears before the what-happened line', blockedRich.indexOf('Mã lỗi:') < blockedRich.indexOf('Task bị chặn'));
  const blockedNoCode = buildTelegramText({ event: 'TASK_BLOCKED', session: sess, objective: 'x' });
  falsy('G3 no invented error code when none supplied', blockedNoCode.includes('Mã lỗi:'));
  const startedNoBlockedLabel = buildTelegramText({ event: 'TASK_STARTED', session: sess, objective: 'keep generic label', errorCode: 'IGNORED' });
  tru('G3 non-blocked keeps generic Mục tiêu label', startedNoBlockedLabel.includes('Mục tiêu: keep generic label') && !startedNoBlockedLabel.includes('Mục tiêu thực tế:'));
  falsy('G3 error code only rendered for TASK_BLOCKED', startedNoBlockedLabel.includes('Mã lỗi:'));
...
// ---- H. TASK_BLOCKED projection: real objective + session error code --------
{
  const sd = path.join(TMP, 'stH');
  const sess = { ...makeSession(sd, 71), errorCode: 'STEP_THREW:boom' };
  let sentText = null;
  const capture = (_file, _args, opts) => { sentText = opts && opts.input; return { error: 0, stdout: JSON.stringify({ ok: true, status: 'API_ACCEPTED', messageId: 5150, chatId: 1 }) + '\n', stderr: '' }; };
  const r = dispatchLifecycleEvent({ session: sess, event: 'TASK_BLOCKED', stateDir: sd, spawn: capture, allowNonCanonicalStateRoot: true, note: 'blocked now' });
  eq('H dispatch accepted', r.status, 'API_ACCEPTED');
  tru('H worker received text', typeof sentText === 'string' && sentText.length > 0);
  const parsed = JSON.parse(String(sentText).trim().split(/\r?\n/).pop());
  tru('H dispatched text carries session error code', String(parsed.text || '').includes('Mã lỗi: STEP_THREW:boom'));
  // explicit errorCode param wins over session field
  let sent2 = null;
  const capture2 = (_file, _args, opts) => { sent2 = opts && opts.input; return { error: 0, stdout: JSON.stringify({ ok: true, status: 'API_ACCEPTED', messageId: 5151, chatId: 1 }) + '\n', stderr: '' }; };
  const sd2 = path.join(TMP, 'stH2');
  const sess2 = { ...makeSession(sd2, 72), errorCode: 'SESSION_CODE' };
  dispatchLifecycleEvent({ session: sess2, event: 'TASK_BLOCKED', stateDir: sd2, spawn: capture2, allowNonCanonicalStateRoot: true, errorCode: 'PARAM_CODE' });
  const parsed2 = JSON.parse(String(sent2).trim().split(/\r?\n/).pop());
  tru('H explicit errorCode param wins', String(parsed2.text || '').includes('Mã lỗi: PARAM_CODE') && !String(parsed2.text || '').includes('SESSION_CODE'));
}
